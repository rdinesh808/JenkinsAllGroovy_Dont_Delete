...
def call(body) {
	...
	def VerficationEnvironment = config.VerficationEnvironment
	def SettingnEnvironment = config.SettingnEnvironment
    ...
    node {
                   ...
                   // stage('code excecution'){
                    if (config.java == 8) {
                        buildEnvVars.add("JAVA_HOME=${tool 'jdk8u121'}")
						   ...
						   withEnv(buildEnvVars) {
						    def clean="mvn clean install"
												 ...
												 def mvnUpdateCmd="mvn --settings settings.xml liquibase:update -Dliquibase.changeLogFile=db.changelog.xml -P${SettingnEnvironment}"
										         def mvnCleanupCmd="mvn --settings settings.xml liquibase:update -Dliquibase.changeLogFile=db.changelog.xml -P${SettingnEnvironment}"
												 def mvnDbVerificationCmd="mvn exec:java -Dspecification.operation=\"dbVerification\" -Dspecification.environment=\"${VerficationEnvironment}\" -Dspecification.clientConfig=\"${environment}\""
												 ...
												 def mvnXmlCleanupModificationCmd="mvn exec:java -Dspecification.operation=\"xmlModification\" -Dspecification.environment=\"CleanupScript\\${environment}_Cleanup\" -Dspecification.buildNo=\"${env.BUILD_NUMBER}\" -Dspecification.author=\"Transformer\""   
                       ...
                       // stage('Restarting dashboard'){							
						//	echo "Starting restart job"
			            //    def restartJob="EnvironmentRestart-${VerficationEnvironment}"
			             //   echo "${restartJob}"
		 ...
		 stage('Deploy Dashboard'){
			echo "Starting DeployV2-6.0.3.2-Prod-Escrow-DashboardServer job..."
			def dailyVer="DeployV2-6.0.3.2-Prod-Escrow-DashboardServer"
			echo "${dailyVer}"
			build job: "${dailyVer}"
			}
		 
		 stage('Config Verification in UI'){
			...
			echo "Starting Escrow_DailyProdVerification job..."
			def dailyVer="Escrow_DailyProdVerification"
			echo "${dailyVer}"
			build job: "${dailyVer}"
			}
		...
		bat 'git branch --set-upstream-to=origin/6.0.4 6.0.4'
		 ...
		 bat 'git pull'
		  bat 'git push origin HEAD:6.0.4'
				...
				//echo "Starting ClientConfigurationBackToInitialState job"
				//def dailyVer="ClientConfigurationBackToInitialState"
				//echo "${dailyVer}"
				//build job: "${dailyVer}"
