...
GitBuildTask task = new GitBuildTask(env)
        ...
        try{
			 ...
			 if (config.env=="Dev")
			 {
			            def PS_deploy = "D:/jenkins_utils/deployment_v2/ps/DevRestart.ps1 -env ${config.env} -environmentType ${config.environmentType} -restartList '${apps}'"
                bat script: "${getPSCmd(PS_deploy)}"
			 }else if (config.env=="QA"){
			            def PS_deploy = "D:/jenkins_utils/deployment_v2/ps/QARestart.ps1 -env ${config.env} -environmentType ${config.environmentType} -restartList '${apps}'"
                bat script: "${getPSCmd(PS_deploy)}"
			 }else if(config.env=="CCN1"){
						def PS_deploy = "D:/jenkins_utils/deployment_v2/ps/EsRestart.ps1 -env ${config.env} -environmentType ${config.environmentType} -restartList '${apps}'"
                bat script: "${getPSCmd(PS_deploy)}"
			 }	else{
			          echo "Noting to restart" 
