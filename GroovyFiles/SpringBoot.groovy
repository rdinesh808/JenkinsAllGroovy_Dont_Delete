...
PS_COPY  = "D:/ps/htmltosharesspringboot.ps1" + " -jobPath '${currentjobPath}' -jobName '${env.JOB_NAME}'"
...
def reportFile="xcopy /s /Q /F /Y \"${projectWorkspace}/CustomizedReports/*.html\" \"${reportpath}/SpringBoot/\""
bat "${reportFile}"
// if(!status){
// try{
// ScreenShot_COPY  = "D:/ps/screenshotcpspringboot.ps1" + " -jobPath '${currentjobPath}' -jobName '${env.JOB_NAME}'"
...
DEFAULT_CONTENT="SpringBoot execution failed for ${env.JOB_NAME}"
}    
if (status){    
...
bat 'git add -A'
if("${testSuitePath}".length() >0){
def gitcmd="git commit -m \"${config.testSuitePath}_Result_Update\""
bat "${gitcmd}" 	
}else{
def gitcmd="git commit -m \"${config.testSuiteCollectionPath}_Result_Update\""
bat "${gitcmd}" 	
}
